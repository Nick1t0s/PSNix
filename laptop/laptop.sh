...
# ---- Hyprland: окружение + waybar + терминальный файл-пикер (yazi) ----
task_hyprland() {
  local repo configs host=laptop
  ...
  if [ ! -x "$HOME/.local/libexec/xdg-desktop-portal-termfilechooser" ]; then
    rm -rf /tmp/tfc-build
    git clone --depth 1 https://github.com/hunkyburrito/xdg-desktop-portal-termfilechooser /tmp/tfc-build || return 1
    ( cd /tmp/tfc-build && meson setup build --prefix="$HOME/.local" && ninja -C build install ) || return 1
  fi
  ...
  if [ ! -x "$HOME/.local/libexec/file_manager_dbus" ]; then
    rm -rf /tmp/fm1-build
    git clone --depth 1 https://github.com/boydaihungst/org.freedesktop.FileManager1.common /tmp/fm1-build || return 1
    ( cd /tmp/fm1-build && meson setup build --prefix="$HOME/.local" && ninja -C build install ) || return 1
  fi
